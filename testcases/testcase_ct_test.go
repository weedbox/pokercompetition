...
func TestCT(t *testing.T) {
	logData := func(t *testing.T, msg string, jsonPrinter func() (*string, error)) {
		json, _ := jsonPrinter()
		t.Logf("\n===== [%s] =====\n%s\n", msg, *json)
	}
	...
	for _, joinPlayer := range joinPlayers {
		...
		// Step 2: 加入桌
		newTable, err := tableEngine.PlayerJoin(table, pokertablemodel.JoinPlayer{PlayerID: joinPlayer.PlayerID, RedeemChips: joinPlayer.RedeemChips})
	...
	// 開打
	// Step 1: 賽事內所有桌次開打
	newTables := make([]*pokertablemodel.Table, 0)
	...
	// Make TableClosed
	competition.State.Tables[0].State.Status = pokertablemodel.TableStateStatus_TableGameClosed
